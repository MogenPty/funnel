...
export const WaitlistForm: React.FC<WaitlistFormProps> = ({ industry, themeColor }) => {
  ...
  if (submitted) {
    return (
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-8 text-center shadow-xl border border-slate-200 dark:border-slate-800 animate-in fade-in zoom-in duration-300">
        <div className={`w-16 h-16 bg-${themeColor}-100 dark:bg-${themeColor}-900/30 rounded-full flex items-center justify-center mx-auto mb-6`}>
          <svg className={`w-8 h-8 text-${themeColor}-600`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
          </svg>
        </div>
        <h3 className="text-2xl font-bold mb-2">You're on the list!</h3>
        <p className="text-slate-600 dark:text-slate-400">
          Thank you for your interest in Mogen. We'll reach out to <strong>{formData.email}</strong> as soon as we launch our beta.
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-900 rounded-2xl p-8 shadow-xl border border-slate-200 dark:border-slate-800">
      <h3 className="text-2xl font-bold mb-6 text-center">Join the Beta Waitlist</h3>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium mb-1 opacity-70">Work Email</label>
            <input 
              required
              type="email" 
              className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 outline-none transition"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input 
              readOnly
              value={industry === 'FUNERAL' ? 'Funeral Services' : 'Legal Practice'}
          ...
          <div>
            <label className="block text-sm font-medium mb-1 opacity-70">Expected Monthly Budget</label>
            <select 
              className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 outline-none transition"
        ...
        <div>
          <label className="block text-sm font-medium mb-1 opacity-70">Message (Optional)</label>
          <textarea 
            rows={3}
            className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 outline-none transition"
